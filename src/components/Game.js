...
export class Game extends Component {
  ...
  componentDidMount = () => {
    socket.on("connect", () => {
      setTimeout(() => {
        socket.emit("user", { user: this.props.auth0_id });
      }, 500);
    });
  ...
  render() {
    ...
    const displayChat = chatMessages.map(chats => {
      return (
        <div className="chatMessage" key={`${chats.key}${chats.user}`}>
          <div className="chatUser" style={{ color: this.props.color }}>
            {chats.user}:
          </div>
    ...
    const data = {
      ...
      datasets: [
        {
          data: chartPercent,
          backgroundColor: ["white", timerColor],
          hoverBackgroundColor: ["white", timerColor]
        }
      ]
    };
    let windowWatch = 115;

    return !this.props.user.length ? (
      ...
      >
        <div className="gameContent">
          <div className="gameInfoContainer">
            ...
            <div className="bankContainer">
              <div>Bank:</div>
              <div
                className="bankNumber"
                style={{ fontSize: this.state.bankFont }}
              >
                {this.props.bank}
              </div>
            </div>
          ...
          <div className="cardImageContainer">
            <img
              className="cardImage"
              src={fromServer && fromServer.image}
              onError={e => {
          ...
          <div className="betContainer">
            ...
            <div className="betButtonContainer">
              <div className="betButtons">
                <button
                  onClick={() => this.lowerBet()}
                  disabled={buttonDisable}
                >
                  Bet -5
                </button>
                <button
                  onClick={() => this.raiseBet()}
                  disabled={buttonDisable}
                >
                  Bet +5
                </button>
              ...
              <div className="betButtons">
                <button
                  onClick={() => this.placeBet("low")}
                  disabled={buttonDisable}
                >
                  Lower
                </button>
                <button
                  onClick={() => this.placeBet("high")}
                  disabled={buttonDisable}
                >
                  Higher
                </button>
        ...
        <div className="chatContent">
          <div className="chatWindow">
            {chatMessages.length ? displayChat : <div />}
          </div>
          <div className="chatInput">
            ...
            <div className="chatButtons">
              <button onClick={() => this.sendMessage(messageText)}>
                Send
              </button>
              <button onClick={() => this.sendMessage("players?")}>
                Players
              </button>
            </div>
          </div>
